...
describe User do
  ...
  describe '#confirmed_gift_requests' do
    context 'When there are existing gift requests for gift_items' do
      ...
      let!(:gift_items_1) { create_list :gift_item, 3, user: user }
      let!(:gift_items_2) { create_list :gift_item, 2, user: user2 }
      let!(:gift_items_3) { create_list :gift_item, 4, user: user3 }
      let!(:gift_request) do
        create :gift_request, user: user, organization: organization,
                              gift_item: gift_items_1[0], done: false
      end
      let!(:gift_request1) do
        create :gift_request, user: user, organization: organization1,
                              gift_item: gift_items_1[0], done: true
      end
      let!(:gift_request2) do
        create :gift_request, user: user, organization: organization2,
                              gift_item: gift_items_1[2], done: true
      end
      let!(:gift_request3) do
        create :gift_request, user: user2, organization: organization1,
                              gift_item: gift_items_2[1], done: true
      end
      let!(:gift_request4) do
        create :gift_request, user: user3, organization: organization2,
                              gift_item: gift_items_3[1], done: false
      end
      let!(:gift_request5) do
        create :gift_request, user: user3, organization: organization2,
                              gift_item: gift_items_3[1], done: true
      end

      it 'returns 2 gift requests' do
  ...
  describe '#pending_gift_requests' do
    context 'When there are existing gift requests for gift_items' do
      ...
      it 'returns two gift requests' do
        expect(user.pending_gift_requests.count).to eq 2
      end
      it 'returns gift_requests pending' do
        expect(user.pending_gift_requests
          .all? { |org_gift_req| org_gift_req[:gift_requests].all? { |d| d[:done] } }).to be false
      end
      it 'returns gift_requests for the selected user' do
        expect(user.confirmed_gift_requests
          .all? { |org_gift_req| org_gift_req[:gift_requests].all? { |d| d.user_id == user.id } })
